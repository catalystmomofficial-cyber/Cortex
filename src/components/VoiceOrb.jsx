...
// sphere (fibonacci), gently rotating, with a flowing surface displacement that
// swells while listening. Depth controls brightness/size so it reads as a
// glowing 3D orb of fine gold filaments.
export default function VoiceOrb({ active = false, size = 240 }) {
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      for (let i = 0; i < N; i++) {
        ...
        // Flowing surface displacement (cheap pseudo-noise from layered sines).
        const n =
          Math.sin(x * 3.2 + t * 1.6) *
          Math.cos(y * 3.0 - t * 1.2) *
          Math.sin(z * 3.4 + t)
        ...
        // Champagne -> bright gold by depth.
